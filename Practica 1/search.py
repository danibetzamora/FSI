...
def graph_search(problem, fringe):
    ...
    while fringe:
        ...
        if problem.goal_test(node.state):
            print("Nodos visitados: {}".format(visited))
            return node
...
def branch_and_bound_graph_search(problem):
    """Select the lower cost path."""
    return graph_search(problem, lower_path_cost())

def branch_and_bound_understimation_graph_search(problem):
    """Select the best path with heuristic"""
    return graph_search(problem, lower_path_cost_heuristic(problem))
